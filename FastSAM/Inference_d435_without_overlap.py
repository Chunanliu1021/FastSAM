...
class FastSAM_with_realsense(object):
  ...
  def image_cb( self, image_msg, info_msg, depth_msg):
    try:
        ...
        #沒有pixel會是0
        self.image_depth[np.isnan(self.image_depth)] = 0.
        nonzero_mask = (self.image_depth != 0 )
        #np.iinfo(self.image_depth.dtype).max為 65535
        self.image_depth[~nonzero_mask] = np.iinfo(self.image_depth.dtype).max

        self.image_time = image_msg.header.stamp



        ###用來檔深度大於特定長度與nan(0)的點(過濾掉過遠的點)###
        # 給450～500 就差不多了
        depth_zero_mask = (self.image_depth > 1000) | (self.image_depth == 0)

        self.image_masked[depth_zero_mask] = 0
        ####################################################
    ...
    try:
        ...
        self.result_pub.publish(self.bridge.cv2_to_imgmsg(self.result_img, "bgr8"))
    except CvBridgeError as e:
        print(e)
...
def main():
    rospy.init_node('Inference_d435_without_overlap', anonymous=True)
    args = parse_args()
