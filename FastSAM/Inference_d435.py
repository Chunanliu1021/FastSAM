...
class FastSAM_with_realsense(object):
  ...
  def image_cb( self, image_msg, info_msg, depth_msg):
    ...
    if (self.number_of_result == 5):
        ...
        try:
            # 這邊只會pub一次(因為self.number_of_result沒重製)
            print("sending final mask img")
            self.final_mask_result_pub.publish(self.bridge.cv2_to_imgmsg(final_result, "bgr8"))
            
            # 用於驗證語意分割
            cv2.imwrite("./output/mask.jpg", final_result)
        except CvBridgeError as e:
            print(e)
...
def parse_args():
    ...
    parser.add_argument(
        "--model_path", type=str, default="/home/user/FastSAM_widden/FastSAM/weights/new_weight/FastSAM-x.pt", help="model"
    )
    parser.add_argument(
