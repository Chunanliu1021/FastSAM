...
class FastSAM_with_realsense(object):
  ...
  def image_cb( self, image_msg, info_msg, depth_msg):
    ...
    #   self.image_depth[np.isnan(self.image_depth)] = 0.
      ...
      self.image_masked[depth_mask] = 0
      self.image_masked[zero_mask] = 0
      ####################################################
    ...
    # # 将NumPy数组转换为OpenCV图像
    # self.image_rgb = cv2.cvtColor(self.image_rgb, cv2.COLOR_RGB2BGR)

    
    # # 顯示影像
    # cv2.imshow('result', self.image_rgb)
    # cv2.waitKey(3)
